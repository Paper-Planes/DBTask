...
window.onload = function() {
    ...
    var phNumberMsg = document.getElementById("phNumberMsg");
...
// 对保单号进行验证
    ...
    phNumber.onkeyup = function() {
        if (!number(this.value)) {
            phNumberMsg.innerHTML = '<a style="color: orangered;">账号中含有除数字和字母之外的非法字符</a>';
        }
        else {
            phNumberLength = getLength(this.value);
            if (phNumberLength < 4) {
                phNumberMsg.innerHTML = "当前" + phNumberLength + "个字符，"+'<a style="color: orangered;">账号名过短</a>';
            } else if (phNumberLength <= 20) {
                phNumberMsg.innerHTML = "当前" + phNumberLength + "个字符，账号名格式正确";
            } else {
                phNumberMsg.innerHTML = "当前" + phNumberLength + "个字符，"+'<a style="color: orangered;">账号名过长</a>';
            }
        }
    }
    //对员工进行验证
